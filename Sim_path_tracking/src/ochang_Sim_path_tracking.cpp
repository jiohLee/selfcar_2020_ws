...
class PurePursuit
{
    ...
    private:
        ...
        ros::Publisher cmdvel_pub, marker_pub,moraiVel_pub;
        ...
        int controller_freq;
        double go_vel_from, stop_vel_from, cross_road_vel_from,slow_down_for_traffic_light_vel_from,static_obs_vel_from,outbreak_obs_vel_from,parking_search_vel_from,parking_in_vel_from,parking_wait_vel_from,parking_out_vel_from,finish_vel_from,slow_down_vel_from,calibration_from;
        double go_vel_to, stop_vel_to, cross_road_vel_to,slow_down_for_traffic_light_vel_to,static_obs_vel_to,outbreak_obs_vel_to,parking_search_vel_to,parking_in_vel_to,parking_wait_vel_to,parking_out_vel_to,finish_vel_to,slow_down_vel_to,calibration_to;
...
PurePursuit::PurePursuit()
{
    ...
    pn.param("cail_mode",cali_mode,false);
...
/*****************************************************************************************/
   ...
   slow_down_vel_from = 100.0 *velocity_gain;
   slow_down_vel_to = 110.0 *velocity_gain;

   calibration_from = 100.0 *velocity_gain;
   calibration_to = 110.0 *velocity_gain;
...
*/
        ...
        if(foundForwardPt)
        {
            ...
            if(!this->goal_reached)
            {
                    ...
                    else if(this->state.data=="parking")
                    {
                     ...
                     else if(this->state.data=="slow_down")
                    {
                         this->velocity = slow_down_vel_to-vel_penalty;
                         if(this->velocity <slow_down_vel_from)
                            this->velocity=slow_down_vel_from;

                    }
                    else if(this->state.data=="calibration")
                    {
                         this->velocity = calibration_to-vel_penalty;
                         if(this->velocity <calibration_from)
                            this->velocity= calibration_from;
    ...
    {
        if(this->cali_mode == true && this->state.data=="calibration"){
            this->steering = 0;
        }
